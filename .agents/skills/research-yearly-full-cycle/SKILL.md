...
- Use Codex goal only when the user explicitly requests goal tracking or invokes a default prompt that says to create a goal.
...
If the user explicitly requested goal tracking:

1. Create one Codex goal with objective `完成 <topic> 的全年全流程调研、独立评分和至少一轮必要迭代`.
2. Use the goal as status tracking only; keep durable data in reports and the knowledge base.
3. Check goal status between stages when useful.
4. Mark the goal complete only after all required stage outputs and required re-evaluations are finished.
5. Mark the goal blocked only when the same blocker repeats for at least three consecutive goal turns and no meaningful progress remains.
